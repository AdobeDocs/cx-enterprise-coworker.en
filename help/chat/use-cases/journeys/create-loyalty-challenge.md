---
title: Create Loyalty Challenges and Insights
description: Learn how Coworker helps create and manage loyalty challenges, automate audience selection, and surface insights.
role: User
level: Beginner
doc-type: Feature Video
duration: 499
last-substantial-update: 2026-08-12T00:00:00.000Z
jira: KT-22312
product_v2:
  - id: fdae8433-07cd-42e7-acce-738afe63f6bb
    internal-label: CX Enterprise Coworker
feature_v2:
  - id: fdae8433-07cd-42e7-acce-738afe63f6bb
    internal-label: CX Enterprise Coworker
---

# Adobe Journey Optimizer & Coworker - create a loyalty challenge and surface insights

In this demonstration, you'll see how Adobe Journey Optimizer (AJO) Loyalty is used in Coworker to simplify and accelerate loyalty program management.
 
The video showcases the Loyalty plugin and its Challenge Management skill, an AI-powered capability that helps marketers create, edit, and manage loyalty challenges using natural language instead of manually configuring every setting in the user interface.

>[!VIDEO](https://video.tv.adobe.com/v/3496528/?learn=on&enablevpops)

## See also

- [Journey Agent](https://experienceleague.adobe.com/en/docs/cx-enterprise-ai/experience-cloud-ai/agents/ajo-agent), for other AJO AI capabilities available through Coworker Chat, such as journey creation, analysis, and simulation.
- [Loyalty Challenge Management](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/get-started/essentials/ajo-coworker-skills#loyalty-challenge-management){target="_blank"} in the Adobe Journey Optimizer documentation.
- [More Coworker Chat use cases](../overview.md#journeys)
